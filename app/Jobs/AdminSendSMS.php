...
class AdminSendSMS implements ShouldQueue
{
    ...
    {
        $status = [];
        $awsId = $msgId = $message = $smsId = $supplier = $userId = $getResponse = $sendResponse = '';
        $param = $this->param;
        ...
        if(!empty($supplier) && !empty($phone) && !empty($message)){
            ...
            try {
                switch ($supplier) {
                    case 'twilio':
                        ...
                        $sendResponse = json_encode($sendResponse,JSON_UNESCAPED_UNICODE);
                        break;
            ...
            empty($sendResponse) ? $sendResponse = "To: $phone" : '';
